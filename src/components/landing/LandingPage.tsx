...
export function LandingPage() {
  return (
    <div className="landing-shell relative min-h-dvh overflow-x-hidden text-foreground">
      <LandingBackground />

      <div className="relative z-10">
        <nav className="glass-nav sticky top-0 z-20 flex items-center justify-between px-6 py-4 md:px-10">
        ...
        <section className="mx-auto max-w-5xl px-6 pt-16 pb-20 text-center md:pt-24">
          ...
          {/* CSS inbox mockup */}
          <div className="glass-panel mx-auto mt-14 max-w-3xl overflow-hidden text-left ring-1 ring-primary/10">
            ...
            <div className="divide-y divide-border/50">
              {[
                ...
                <div
                  key={r.subj}
                  className="flex cursor-pointer items-center gap-3 px-4 py-3 text-sm transition-all duration-200 hover:bg-muted/40"
                >
                  ...
                  <span className="ml-auto shrink-0 rounded-full bg-muted/60 px-2 py-0.5 text-[10px] uppercase tracking-wide text-muted-foreground backdrop-blur-sm">
                    {r.tag}
                  </span>
                </div>
              ))}
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {FEATURES.map((f) => (
                <div
                  key={f.title}
                  className="glass-panel cursor-default p-6 transition-all duration-200 hover:border-primary/20 hover:shadow-md"
                >
        ...
        <section className="px-6 py-20 md:px-10">
          <div className="mx-auto grid max-w-5xl items-center gap-12 md:grid-cols-2">
            <div className="grid grid-cols-3 gap-6 text-center">
              {[
                { value: 184, suffix: "", label: "emails triaged daily" },
                { value: 92, suffix: "%", label: "classified correctly" },
                { value: 3, suffix: "h", label: "saved per week" },
              ].map((stat) => (
                <div
                  key={stat.label}
                  className="glass-panel flex aspect-square cursor-default flex-col items-center justify-center transition-all duration-300 hover:scale-105"
                >
            ...
            <div className="mt-12 grid gap-8 md:grid-cols-3">
              {STEPS.map((s) => (
                <div key={s.n} className="text-center">
                  <div className="mx-auto flex size-10 items-center justify-center rounded-full bg-primary text-sm font-semibold text-primary-foreground ring-4 ring-primary/10">
                    {s.n}
                  </div>
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {SHORTCUT_GROUPS.map((g) => (
                <div key={g.group} className="glass-panel p-6">
                  <h3 className="mb-4 text-sm font-semibold text-primary">{g.group}</h3>
                  <ul>
                    {g.items.map((s) => (
                      <li
                        key={s.label}
                        className="flex cursor-alias items-center justify-between gap-3 rounded-md p-2 transition-all duration-200 hover:bg-muted/40"
                      >
                        ...
                        <span className="flex shrink-0 items-center gap-1">
                          {s.keys.map((k) => (
                            <kbd
                              key={k}
                              className="inline-flex min-w-6 items-center justify-center rounded-md border border-b-2 border-border/60 bg-background/50 px-1.5 py-0.5 text-xs font-medium text-foreground shadow-sm backdrop-blur-sm"
                            >
                              {k}
                            </kbd>
                          ))}
        ...
        <footer className="border-t border-border/40 p-4 text-center text-sm text-muted-foreground md:px-10">
